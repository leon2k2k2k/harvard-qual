...
We also have the Arzela-Ascoli
To show that a family of continuous functions on the interval uniformly converges (this is use for example to show that the sequence converges in $L^p$ norm for any $p$), it gives sufficient conditions:
...
Unifomrly bounded means that exists $M$ such that $$
f_n(x) \leq M
$$
for all $n$ and $x$. Uniformly equicontinuous means that for every $\epsilon > 0$ there exists a delta such that for all $x, y$ with $|x - y| < \delta$ and all $n$ we have 
$$
|f_n(x) - f_n(y)| < \epsilon
$$

To show that a function is analytic, it is suffice to show that $|f^{(j)}(x)| \leq C^{k + 1} k!$.
